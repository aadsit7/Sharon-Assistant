...
//
// The web app never hardcodes the key: getApiKey() reads the visitor's
// passphrase from their own browser fresh on EVERY request, so unlocking
// (or re-entering) it takes effect immediately, with no reload.
...
async function call(action, payload, signal) {
  const body = { api_key: getApiKey(), action, payload };
